...
class ConfigManager:
    ...
    def __init__(self, config_dir: Optional[Path] = None, cache_ttl: int = 300, logger=None):
    ...
    def _load_config_value(self, key_path: str, default: Any, agent_config: str = None) -> Any:
        ...
        try:
            ...
            # 如果多文件配置为空，尝试单一文件
            if not config_data and self._config_path and self._config_path.exists():
    ...
    def load_multi_file_config(self) -> Dict[str, Any]:
        ...
        if not merged_config and self._config_path and self._config_path.exists():
    ...
    def enable_hot_reload(self, callback: Optional[Callable] = None) -> None:
        ...
        if not _HAS_WATCHDOG:
            if self._logger:
                self._logger.warning("watchdog not installed, hot reload disabled")
            return
        ...
        if self._config_dir is None:
            if self._logger:
                self._logger.warning("config_dir not set, hot reload disabled")
            return
    ...
    def _on_config_changed(self, event) -> None:
        ...
        # 检查是否是配置文件
        config_files = ['config.yaml', 'base.yaml']
        if self._config_dir:
            try:
                config_files += [f for f in os.listdir(self._config_dir) if f.endswith('.yaml')]
            except OSError:
                pass
...
def get_config_value(key_path: str, default=None, agent_config: str = None) -> Any:
    """快捷方法：获取配置值"""
    return get_config_manager().get_config_value(key_path, default, agent_config)
