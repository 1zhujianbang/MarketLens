...
class KeyManager:
    ...
    def __init__(self, key_store_path: Optional[Path] = None, master_key: Optional[str] = None):
        ...
        if key_store_path is None:
            try:
                ...
                # 回退到默认路径
                key_store_path = Path(__file__).parent.parent.parent / "config" / ".key_store.enc"
    ...
    def store_api_key(self, service_name: str, api_key: str, metadata: Optional[Dict[str, Any]] = None):
        ...
        existing = (data.get("keys") or {}).get(service_name) or {}
        existing_hash = existing.get("key_hash")
        existing_meta = existing.get("metadata") if isinstance(existing.get("metadata"), dict) else {}

        # 若密钥与元数据均未变化，则跳过写盘与日志，避免刷屏
        if existing_hash == key_hash and existing_meta == incoming_meta:
            self.logger.debug(f"API key for {service_name} unchanged; skip storing")
            return
        ...
        data["keys"][service_name] = {
            ...
            # 若已存在则保留原 created_at / last_used，避免每次"迁移"都改写时间
            "created_at": existing.get("created_at") or self._get_timestamp(),
