...
def safe_unlink(file_path: Path, log_func=None, log_prefix: str = "", missing_ok: bool = True) -> bool:
    """
    安全删除文件，带日志记录
    ...
    try:
        if file_path.exists():
            ...
            if log_func:
                log_func(f"🗑️ 删除{log_prefix}文件: {file_path}")
            return True
        return False
    except Exception as e:
        if log_func:
            log_func(f"⚠️ 删除{log_prefix}文件失败 {file_path}: {e}")
        return False


def safe_unlink_multiple(file_paths: List[Path], log_func=None, log_prefix: str = "") -> int:
    """
    安全删除多个文件
    ...
    for file_path in file_paths:
        if safe_unlink(file_path, log_func, log_prefix):
            deleted_count += 1
...
def cleanup_temp_files(temp_dir: Path, pattern: str = "*", max_age_hours: int = 24, log_func=None) -> int:
    """
    清理临时目录中的过期文件
    ...
    for file_path in temp_dir.glob(pattern):
        if file_path.is_file():
            ...
            if file_age > max_age_seconds:
                if safe_unlink(file_path, log_func, "过期临时"):
                    deleted_count += 1
...
class AsyncFileOperations:
    ...
    async def read_json_async(file_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        if not HAS_AIOFILES:
            # 回退到同步操作
            return read_json_sync(file_path)
    ...
    async def write_json_async(
        ...
        if not HAS_AIOFILES:
            # 回退到同步操作
            write_json_sync(file_path, data, indent, ensure_ascii)
            return
