...
"""Generated from `pages/2_Pipeline_Builder.py`.
...
from src.web.config import PROJECT_ROOT, DATA_DIR
...
def render() -> None:
    ...
    st.title("Pipeline")
    if_advanced_console = st.sidebar.checkbox("进入高级控制台", value=False)
    ...
    def execute_pipeline(pipeline_def):
        """提交任务到后台管理器"""
        if not can_write():
            st.error("当前角色为 viewer：禁止启动流水线（权限占位）。")
            return
        if task_manager.is_running:
            st.warning("⚠️ 已有任务正在运行。请等待完成后再试。")
            return

        # Baseline snapshot for run change pack (UI thread)
        try:
            st.cache_data.clear()
        except Exception:
            pass
        st.session_state["_run_baseline"] = {
            "entities": list((utils.load_entities() or {}).keys()),
            "events": list((utils.load_events() or {}).keys()),
        }

        history_idx = append_history(pipeline_def)
        success = task_manager.start(pipeline_def, history_idx=history_idx)
        if success:
            st.toast("🚀 Task started in background!")
            st.rerun()
        else:
            # 启动失败则回滚刚追加的历史记录，避免出现“running 但没启动”的脏数据
            try:
    ...
    if not if_advanced_console:
        ...
        st.divider()
        secondary_col = st.columns([1,6,6,8,1])
        with secondary_col[1]:
            limit = st.number_input("每源抓取条数", min_value=1, max_value=10, value=10, step=1, help="（API上限通常为10）")
            if fetch_mode == "关键词搜索（推荐）":
                kw_raw = st.text_input(
                    "关键词",
                    placeholder="例如：Nvidia, AI 芯片, ETF",
                    help="多个用逗号/空格分隔,建议 1~3 个关键词",
                )
                keywords = [k.strip() for k in kw_raw.replace("，", ",").replace(";", ",").split(",")]
                # also split by spaces if user didn't use commas
                if len(keywords) == 1:
                    keywords = [k.strip() for k in kw_raw.split(" ") if k.strip()]
                keywords = [k for k in keywords if k]
        with secondary_col[2]:
            enable_report = st.checkbox("生成报告", value=True)
            allow_append_forms = st.checkbox("仅新增模式：允许追加 original_forms", value=True)
        with secondary_col[3]:
            now_utc = datetime.now(timezone.utc)
            from_val = None
            to_val = None
            if time_mode in ("最近24h", "最近7d"):
                ...
                d_from, d_to = st.date_input(
                    "自定义日期区间",
                    value=(now_utc.date(), now_utc.date()),
                    help="//",
                )
                try:
        ...
        if not selected_sources:
            ...
            with st.expander(f"✅ 已选择数据源：{len(selected_sources)} 个（展开查看）", expanded=False):
                st.write(", ".join(selected_sources))

        need_kw = (fetch_mode == "关键词搜索（推荐）") and (not keywords)
        if need_kw:
            st.warning("关键词搜索模式下需要至少 1 个关键词。")
        run_btn = st.button(
            "开始运行",
            type="primary",
            use_container_width=True,
            disabled=(not selected_sources) or need_kw,
        )
        if run_btn:
            # Step 1: fetch/search
            if fetch_mode == "关键词搜索（推荐）":
                ...
                # 最新抓取：若提供关键词，则作为 query 过滤（单关键词更合适；多关键词用空格 join）
                    ...
                    "steps": [
                        ...
                        },
                        {
                            ...
                            "inputs": {
                                ...
                                "allow_append_original_forms": bool(allow_append_forms),
                            },
                            "output": "kg_update_result",
                        },
                        {"id": "refresh_kg", "tool": "refresh_knowledge_graph", "inputs": {}, "output": "kg_refresh"},
                    ],
                }
                if enable_report:
                    pipeline_def["steps"].append(
                        {
                            ...
                            "inputs": {"events_list": "$extracted_events", "title": "Incremental Update Report"},
                    ...
                    pipeline_def["steps"].append(
                        {
                            "id": "report",
                            "tool": "generate_markdown_report",
                            "inputs": {"events_list": "$extracted_events", "title": "Initialization Build Report"},
                            "output": "final_report_md",
                        }
                    )
                execute_pipeline(pipeline_def)
