...
from ..infra.serialization import Serializer
    ...
    def clean_event_data(events: List[Dict]) -> List[Dict]:
        ...
        for event in events:
            ...
            cleaned_event = {
                ...
                'event_types': event.get('event_types', []) if isinstance(event.get('event_types'), list) else [],
                'entity_roles': event.get('entity_roles', {}) if isinstance(event.get('entity_roles'), dict) else {},
                'event_start_time': str(event.get('event_start_time', '') or '').strip(),
                'event_start_time_text': str(event.get('event_start_time_text', '') or '').strip(),
                'event_start_time_precision': str(event.get('event_start_time_precision', 'unknown') or 'unknown').strip() or 'unknown',
                'relations': event.get('relations', []) if isinstance(event.get('relations'), list) else [],
