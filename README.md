...
- 用简单脚本或轻量流处理（如 Python + Redis / Kafka）做初步清洗和去重，保证数据不过时（目标延迟 < 5 分钟）。 

2. 图谱构建方式
- 提取新闻中的关键词（如项目名、事件类型）；
- 通过规则或 NLP 模型（如 spaCy、LTP、BERT）扩展实体和关系；
- 图谱存储暂用 JSON 或 SQLite，后期可迁移到 Neo4j 等图数据库。 

3. 智能体协作流程 
- 从新闻中抽取出候选实体 → 用这些实体去检索更多相关新- 闻 → 聚合事件 → 构建“实体-事件”关联 → 推导市场影响；

4. 输出与验证
- 最终目标是生成可解释的推导链（例如：“某国监管收紧 → 某交易所下架代币 → 该代币价格下跌”）；
- 所有预测会记录并回溯验证，避免模型“自说自话”。
