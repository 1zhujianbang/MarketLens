...
### 当前计划
- 首要：时间序列事件图谱构建，实体-关系-实体图谱构建
- 次首要：结构化数据数据库永久存储
- 次要：新闻→实体/事件→图谱链路稳定与质量提升
  - 提取、去重和合并的准确性；避免错误合并造成污染
  - 图谱合并规则、阈值与提示词优化（防止误合并）
  - 并发、缓存、路径一致性治理
- 次要：前端性能与易用性（Pipeline/Graph 页面）
- 规划中：多源行情/结构化数据、推理与强化学习闭环（未实现）
